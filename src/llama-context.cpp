...
bool llama_context::inject_memory_clean(const char * memory_text, int32_t n_layers, int32_t n_ctx_tmp) {
    if (n_ctx_tmp <= 0) {
        LLAMA_LOG_ERROR("%s: invalid n_ctx_tmp=%d\n", __func__, n_ctx_tmp);
        return false;
    }

    // 1. Create temporary context with same model, small cache
    llama_context_params params_tmp = llama_context_default_params();
    params_tmp.n_ctx     = (uint32_t)n_ctx_tmp;
    params_tmp.n_ubatch  = 1;
    params_tmp.n_seq_max = 1;
    params_tmp.embeddings = false;
    params_tmp.offload_kqv = true;

    llama_context * ctx_tmp = llama_init_from_model(
        const_cast<llama_model*>(&model), params_tmp);
    if (!ctx_tmp) {
        LLAMA_LOG_ERROR("%s: failed to create temporary context\n", __func__);
        return false;
    }
    // RAII-style cleanup
    auto free_tmp = [&]() { llama_free(ctx_tmp); };

    // 2. Tokenize
    const llama_vocab * vocab = llama_model_get_vocab(&model);
    int n_tok = llama_tokenize(vocab, memory_text, strlen(memory_text), NULL, 0, true, false);
    if (n_tok == std::numeric_limits<int32_t>::min()) { free_tmp(); return false; }
    if (n_tok < 0) { n_tok = -n_tok; } else { n_tok = 0; }
    if (n_tok <= 0) { free_tmp(); return false; }
    if (n_tok > n_ctx_tmp) n_tok = n_ctx_tmp;  // truncate
    std::vector<llama_token> tokens(n_tok);
    n_tok = llama_tokenize(vocab, memory_text, strlen(memory_text), tokens.data(), tokens.size(), true, false);
    if (n_tok != (int)tokens.size()) { free_tmp(); return false; }

    // 3. Forward on temp context
    llama_batch batch = llama_batch_get_one(tokens.data(), n_tok);
    batch.logits = (int8_t *)calloc((size_t)n_tok, 1);
    batch.logits[n_tok - 1] = 1;
    int ret = llama_decode(ctx_tmp, batch);
    free(batch.logits);
    if (ret != 0) { free_tmp(); return false; }

    // 4. Extract K/V from temp context (replicate inject_memory extraction logic)
    auto * mem = llama_get_memory(ctx_tmp);
    llama_kv_cache * kv_tmp = dynamic_cast<llama_kv_cache *>(mem);
    llama_kv_cache * kv_swa_tmp = nullptr;
    if (!kv_tmp) {
        auto * hybrid = dynamic_cast<llama_memory_hybrid *>(mem);
        if (hybrid) kv_tmp = hybrid->get_mem_attn();
    }
    if (!kv_tmp) {
        auto * iswa = dynamic_cast<llama_kv_cache_iswa *>(mem);
        if (iswa) {
            kv_tmp = iswa->get_base();
            kv_swa_tmp = iswa->get_swa();
        }
    }
    if (!kv_tmp) { free_tmp(); LLAMA_LOG_ERROR("%s: unsupported memory type\n", __func__); return false; }

    auto & hp = model.hparams;
    int last_nh = (int)hp.n_embd_head_k(0);
    int last_nkv = (int)hp.n_head_kv(0);

    // Score layers by K norm
    struct LS { int32_t il; float score; int32_t n_slots; };
    std::vector<LS> layers_info;

    auto score_from_cache = [&](llama_kv_cache * cache, int32_t mil) {
        auto it = cache->map_layer_ids.find(mil);
        if (it == cache->map_layer_ids.end()) return;
        int32_t ikv = it->second;
        if (ikv < 0 || ikv >= (int32_t)cache->layers.size()) return;
        ...
        int nkv_hp = (int)hp.n_head_kv(mil);
        if (nh <= 0 || nkv_hp <= 0) return;
        size_t sz = (size_t)nh * nkv_hp * n_tok;
        std::vector<float> k_buf(sz), v_buf(sz);
        int got = cache->extract_layer_kv(mil, k_buf.data(), v_buf.data(), nh, nkv_hp, n_tok);
        if (got != n_tok) return;
        double sum = 0;
        for (auto x : k_buf) sum += fabs(x);
        layers_info.push_back({mil, (float)(sum / k_buf.size()), n_tok});
    };

    // Base cache layers
    for (int i = 0; i < (int)kv_tmp->layers.size(); i++) {
        score_from_cache(kv_tmp, (int32_t)kv_tmp->layers[i].il);
    }
    // SWA cache layers
    if (kv_swa_tmp) {
        for (int i = 0; i < (int)kv_swa_tmp->layers.size(); i++) {
            int32_t mil = (int32_t)kv_swa_tmp->layers[i].il;
            bool found = false;
            for (auto & l : layers_info) { if (l.il == mil) { found = true; break; } }
            if (found) continue;
            score_from_cache(kv_swa_tmp, mil);
        }
    }

    if (layers_info.empty()) { free_tmp(); LLAMA_LOG_ERROR("%s: no valid layers\n", __func__); return false; }

    std::sort(layers_info.begin(), layers_info.end(),
        [](const LS & a, const LS & b) { return a.score > b.score; });
    if ((int32_t)layers_info.size() > n_layers) layers_info.resize(n_layers);

    // 5. Build flat bank data (same format as inject_memory)
    auto cache_for = [&](llama_kv_cache * base, llama_kv_cache * swa, int32_t mil) -> llama_kv_cache * {
        if (swa && hp.is_swa(mil)) {
            auto it = swa->map_layer_ids.find(mil);
            if (it != swa->map_layer_ids.end()) return swa;
        }
        return base;
    };
    ...
    for (auto & li : layers_info) {
        ...
        memcpy(&f_nh,  &nh,      sizeof(float));
        memcpy(&f_nkv, &nkv_hp,  sizeof(float));
        flat.push_back(f_il); flat.push_back(f_ns);
        flat.push_back(f_nh); flat.push_back(f_nkv);
        size_t off = flat.size();
        flat.resize(off + 2 * sz);
        llama_kv_cache * cache = cache_for(kv_tmp, kv_swa_tmp, li.il);
        int got = cache->extract_layer_kv(li.il, flat.data() + off, flat.data() + off + sz, nh, nkv_hp, ns);
        if (got != ns) { flat.resize(off - 4); } else { last_nh = nh; last_nkv = nkv_hp; }
    }

    if (flat.empty()) { free_tmp(); LLAMA_LOG_ERROR("%s: empty bank\n", __func__); return false; }

    // 6. Load bank on THIS context and set no_rotate
    bool ok = set_kv_bank(flat.data(), flat.size(), last_nh, last_nkv);
    if (ok && kv_bank) {
...
int32_t llama_inject_memory_clean(
        llama_context * ctx,
          const char * memory_text,
              int32_t   n_layers,
              int32_t   n_ctx_tmp) {
    bool res = ctx->inject_memory_clean(memory_text, n_layers, n_ctx_tmp);
    return res ? 0 : -1;
}
